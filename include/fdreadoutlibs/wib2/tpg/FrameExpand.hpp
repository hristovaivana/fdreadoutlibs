...
//==============================================================================
inline __m256i
unpack_one_register(const dunedaq::detdataformats::wib2::WIB2Frame::word_t* first_word)
{
  ...
  for (size_t iframe = 0; iframe < swtpg_wib2::FRAMES_PER_MSG; ++iframe) {
    ...
    for (size_t iblock = 0; iblock < swtpg_wib2::NUM_REGISTERS_PER_FRAME; ++iblock) {
      register_array->set_ymm(
        ...
        swtpg_wib2::unpack_one_register(frame->adc_words +
                                        7 * (iblock + registers_selection * swtpg_wib2::NUM_REGISTERS_PER_FRAME)));
    }
  }
}
