...
public:
  ...
  {
    readoutlibs::TaskRawDataProcessorModel<types::DUNEWIBFirmwareTriggerPrimitiveSuperChunkTypeAdapter>::get_info(
      ci, level);
  ...
  void tp_stitch(rwtp_ptr rwtp)
  {
    ...
    uint offline_channel =
      m_channel_map->get_offline_channel_from_crate_slot_fiber_chan(m_crate_no, m_slot_no, m_fiber_no, m_channel_no);
  ...
  void tp_unpack(frame_ptr fr)
  {
    ...
    while (offset <= num_elem) {
      ...
      auto heap_memory_block =
        malloc(sizeof(dunedaq::detdataformats::fwtp::TpHeader) + nhits * sizeof(dunedaq::detdataformats::fwtp::TpData));
      rwtp_ptr rwtp = static_cast<dunedaq::detdataformats::fwtp::RawTp*>(heap_memory_block);

      ::memcpy(
        static_cast<void*>(&rwtp->m_head), static_cast<void*>(tmpbuffer.data() + 0), 2 * RAW_WIB_TP_SUBFRAME_SIZE);
