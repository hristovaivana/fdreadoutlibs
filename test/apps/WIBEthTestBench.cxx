...
//==============================================================================
__m256i unpack_one_register( dunedaq::detdataformats::wibeth::WIBEthFrame::word_t* first_word)
{
...
RegisterArray<16> unpack( dunedaq::detdataformats::wibeth::WIBEthFrame& frame)
{
    RegisterArray<16> ret;
    for(size_t i=0; i<ret.size(); ++i){
        ret.set_ymm(i, unpack_one_register(frame.adc_words[0]+7*i));
    }
    return ret;
}


bool in_out_test(const std::array<uint16_t, 64>& vals)
{
    dunedaq::detdataformats::wibeth::WIBEthFrame frame;
    // Zero it out first
    std::memset(&frame, 0, sizeof(dunedaq::detdataformats::wibeth::WIBEthFrame));

    for(size_t i=0; i<vals.size(); ++i){
        frame.set_adc(i, 0, vals[i]);
    }
