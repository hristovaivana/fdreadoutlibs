/**
 * @file RSAvx.hpp  class that performs the avx implementation of RS algorithm
 *
 * This is part of the DUNE DAQ Application Framework, copyright 2022.
 * Licensing/copyright details are in the COPYING file that you should have
 * received with this code.
 */
#ifndef TEST_RS_AVX_HPPP_
#define TEST_RS_AVX_HPPP_
...
class RSAvx : public SwtpgBase {


public: 
  explicit RSAvx(bool dump_adc_data, bool dump_hit_data) {
...
unsigned int extract_swtpg_hits_avx(uint16_t* primfind_it, timestamp_t timestamp)   {
    ...
    while (*primfind_it != swtpg_wib2::MAGIC) {
      ...
      for (int i = 0; i < 16; ++i) {
        ...
        //std::cout << "hit_end: " << hit_end[i] << std::endl;

      }
      for (int i = 0; i < 16; ++i) {
        hit_charge[i] = *primfind_it++; // NOLINT(runtime/increment_decrement)
        //std::cout << "hit_charge: " << hit_charge[i] << std::endl;
      ...
      for (int i = 0; i < 16; ++i) {
        //std::cout << "hit_charge: " << hit_charge[i] << "  chan: " << chan[i] << " hit_tover: " << hit_tover[i] << std::endl;
        if (hit_charge[i]>0 && chan[i] != swtpg_wib2::MAGIC) {            
...
void find_hits(const dunedaq::fdreadoutlibs::types::DUNEWIBSuperChunkTypeAdapter* fp, bool first_hit) {
    ...
    if (m_dump_adc_data){
      save_raw_data(registers_array, timestamp, -1, "RSAvx");
    }
    ...
    *m_primfind_dest = swtpg_wib2::MAGIC;
    swtpg_wib2::process_window_rs_avx2(*m_tpg_processing_info, 1);
