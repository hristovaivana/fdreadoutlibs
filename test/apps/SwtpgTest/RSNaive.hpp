/**
 * @file RSNaive.hpp  class that performs the naive implementation of RS algorithm
 *
...
#ifndef TEST_RS_NAIVE_HPPP_
#define TEST_RS_NAIVE_HPPP_
...
class RSNaive : public SwtpgBase {


public: 
  explicit RSNaive(bool dump_adc_data, bool dump_hit_data) {
    m_dump_adc_data = dump_adc_data;
    m_dump_hit_data = dump_hit_data;
  }
  RSNaive(const RSNaive&) = delete;
  RSNaive& operator=(const RSNaive&) = delete;
...
void find_hits(const dunedaq::fdreadoutlibs::types::DUNEWIBSuperChunkTypeAdapter* fp, bool first_hit) {
    ...
    if (m_dump_adc_data){
      save_raw_data(registers_array, timestamp, -1, "RSNaive");
    }
    ...
    *m_primfind_dest = swtpg_wib2::MAGIC;
    swtpg_wib2::process_window_naive_RS(*m_tpg_processing_info);
